const About = () => {
  const frameworks = ['ReactJS', 'Express', 'NodeJs', 'Three.js', 'TailwindCSS', 'Framer Motion']
  const exploring = ['Next.js App Router', 'Animations with Framer Motion', 'Cloud deployments on AWS', 'AI-ML workflows']
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-center animate-on-scroll">
        {/* Left: Decorative asymmetric panel with intro copy */}
        <div className="relative rounded-3xl w-full h-72 sm:h-80 md:h-96 overflow-hidden border border-white/10 bg-[#0F0F12]">
          {/* Subtle grid texture */}
          <div
        ...
        <div className="space-y-4">
          ...
          <div className="group rounded-3xl p-5 md:p-6 bg-[#0F0F12] border border-white/5 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)] relative overflow-hidden w-full">
            ...
            <div className="text-text-secondary text-xs mb-2 transform transition-transform duration-200 group-hover:-translate-y-1.5">Experienced and worked with different</div>
            ...
            <div className="flex flex-wrap gap-2 transform transition-transform duration-200 group-hover:-translate-y-1.5">
              {frameworks.map((t) => (
                <span
                  key={t}
                  className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-text-primary text-xs hover:bg-white/10 hover:border-[#7F5AF0]/50 hover:text-[#7F5AF0] hover:scale-105 hover:-translate-y-0.5 transition-all duration-300 cursor-default inline-block"
                >
                  {t}
                </span>
              ))}
